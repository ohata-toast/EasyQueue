...
| 이름 | 타입 | 유효 범위 | 필수 여부 | 기본값 | 설명 |
|---|---|---|---|---|---|
| seekTypeCode | String | EARLIEST, <br>OFFSET, <br>TAIL | 선택 |  | 메시지 탐색(Seek) 코드<br>* EARLIEST: 가장 처음 오프셋 부터 N개 조회<br>* OFFSET: 지정한 오프셋으로 부터 N개 조회, offset 필수 입력 필요<br>* TAIL: 가장 마지막 오프셋 이후 새로운 메시지를 N개 조회 |
| offset | long | 최소 0 | 선택 |  | 파티션 오프셋(양수값) 지정, seekTypeCode가 OFFSET일 경우 필수 |
| partition | int | 최소 0 | 선택 |  | 파티션 번호(0 ~ 파티션 수-1) 지정 |
| sortDirection | String | DESC, ASC | 선택 | DESC | 정렬 방향 (DESC: 내림차순, ASC: 오름차순) |
| page | int | 최소 1 | 선택 | 1 | 페이지 번호 |
| limit | int | 최소 1, 최대 3,000 | 선택 | 50 | 페이지 당 항목 수 |
...
| 이름 | 타입 | 설명 |
|---|---|---|
| topicMessageList | List | 메시지 목록 |
| topicMessageList[0].key | String | 메시지 Key |
| topicMessageList[0].value | String | 메시지 Value |
| topicMessageList[0].header | List | 메시지 Header (JSON String 형태) |
| topicMessageList[0].header[0].key | String | Header Key |
| topicMessageList[0].header[0].value | String | Header Value |
| topicMessageList[0].partition | int | 파티션 번호 (0 ~ 파티션 수-1) |
| topicMessageList[0].offset | long | 파티션 오프셋 |
